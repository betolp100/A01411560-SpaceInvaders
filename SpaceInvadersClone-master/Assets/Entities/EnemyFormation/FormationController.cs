...
public class FormationController : MonoBehaviour {
	...
	void Start () {
        ...
        SpawnEnemies();	//	Previously SpawnEnemies ();
	}
	...
	void Update () {
		...
		if (AllMembersDead ()) {
            Debug.Log("HOLAA");
                ...
                StartCoroutine(GameObject.Find("BossFormation").GetComponent<FormationBoss>().SpawnUntilFull());
                SpawnEnemies();
            
            //	Previously SpawnEnemies ();
            //SpawnBoss();
        }
	}
    ...
    bool AllMembersDead()
    {
        ...
        if (waveCounter >= 3 && bosses.Length==0)
        {
            levelManager.EnemiesDown();
